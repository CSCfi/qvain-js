...
<template>
	<b-jumbotron class="jumbotron" fluid>
		...
		<b-alert :show="loginError == 'missingcsc'" variant="danger">
			Invalid login. Please make sure that you have a valid CSC account.
		...
		<b-alert :show="loginError == 'missingorg'" variant="danger">
			Invalid login. You currently do not have a home organization set. Please contact CSC Helpdesk to get your home organization sorted out and then come back to log in again.
		</b-alert>

		<b-alert :show="missingLogin" variant="danger">You are currently not logged in. Please login to access Qvain.</b-alert>

		<div v-if="loginError" class="invalid-logout">
				<p><button class="btn btn-info btn-lg" role="button" @click="logout()">Sign out</button></p>
		</div>
